...
下一层现已机器化：[`data/runtime-targets.json`](data/runtime-targets.json) 钉死 DSH 基线和四个社区插件 commit，分别覆盖能力移植、高采用度和静态审查优先级。[运行时兼容矩阵](.github/workflows/runtime-compat.yml) 在 Ubuntu 与 Windows 上构建并缓存该精确 DSH Web 基线，先启动 stock DSH 作为仪器阳性对照，再在全新 `DSH_HOME` 中逐一安装、组合和启动每个 package/OS 组合；每次运行上传一份只创建不覆盖、内容寻址的报告。加载社区代码前会移除凭据形与 CI 控制环境变量，生命周期构建只精确放行该目标声明的一个固定包。通过只证明指定组合兼容，不是安全认证。
